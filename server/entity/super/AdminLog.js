...
AdminLogSchema.statics.queryLog = function (searchWhere, session, cb) {
    if (!CGlobal.isPermission(session.rights, CGlobal.Rights.UserLogs.code)) {
        return cb({message: CGlobal.serverLang('抱歉,你没有 {0} 权限!', CGlobal.Rights.UserLogs.code)});
    }
    ...
    async.auto({
        findShopId: function (callback) {
            callback();
            // Shop.findShopIdListByUser(session, function (err, Ids) {
        ...
        searchLog: ['findShopId', function (findShopIdList, callback) {
            ...
            dao.getPageQuery('AdminLog', where, {}, sortOrder, offset, pageSize, callback);
        }]
    }, function (err, result) {
        if (err) return cb(err);
        cb(null, result.searchLog);
    });
};
